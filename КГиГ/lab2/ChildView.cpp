...
void CChildView::OnRButtonDown(UINT nFlags, CPoint point)
{
	if (startSelected)
	{
		...
		/*if (targetArea.left > targetArea.right)
		{
			std::swap(targetArea.top, targetArea.bottom);
			std::swap(targetArea.right, targetArea.left);
		}*/
		if (targetArea.left > targetArea.right)
		{
			std::swap(targetArea.right, targetArea.left);
		}
		if (targetArea.top > targetArea.bottom)
		{
			std::swap(targetArea.top, targetArea.bottom);
		}
